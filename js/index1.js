...
carrusel.addEventListener("touchend", (e)=>{
   ...
   if (distancia >50){
      cambiarNoticia(actual+1); // si la distancia es mayor, se cambia la noticia
   } else if (distancia < -50){
      cambiarNoticia(actual-1); // lo mismo pero para el otro lado 
   }
});

// evento de flechitas
btnAtras.addEventListener("click",()=>{
   cambiarNoticia(actual-1); //le ponemos de parametro a la funcion el actual - 1 para que vaya hacia atras
});

btnSiguiente.addEventListener("click",()=>{
   cambiarNoticia(actual+1); // lo mismo pero para adelante
});

// evento para los circulos
circulos.forEach((circulo, i)=>{
   circulo.addEventListener("click",()=>{
      cambiarNoticia(i); // le pasamos el indice del circulo al que le dimos click
   });
});
